    ...
    <div className={`inline-flex max-w-full gap-1 overflow-x-auto rounded-full bg-neutral-900 p-1 text-sm font-semibold ${className}`}>
      {tabs.map((t) => (
        <button
          ...
          className={`relative whitespace-nowrap rounded-full px-4 py-1.5 transition-colors ${
        ...
        >
          ...
          {typeof t.badge === 'number' && t.badge > 0 && (
            <span className="ml-1.5 rounded-full bg-white/20 px-1.5 text-xs">{t.badge}</span>
          )}
        </button>
      ))}
    </div>
